<?php
	...
	'
	    ...
	    <tr>
	      <th scope="col">CANCELAR MATERIA</th>
	    </tr>
	';
	$accion = $_POST['accion']	;
	...
	else
	{
		...
		if($accion == 2){
			$resultados = mysqli_query($conexion,"SELECT *  FROM calificaciones INNER JOIN alumno ON calificaciones.documento_est = alumno.documento_est;");
			while($consulta = mysqli_fetch_array($resultados))
			{
			  echo 
			  '
			  <tr>
			  <td><a href="eliminar.php?documento_est='.$consulta['documento_est'].'"><button type="button" class="btn 
              btn-outline-danger" onclick = "return confirmar()">CANCELAR</button></a></td>
		  </tr>
			  ';
			}	
		}
